 ...
 */
unsigned short icsum(u_int32_t source_address, u_int32_t dest_address,
                     u_int16_t tcp_length) {
...
SynAck::SynAck(uint32_t saddr, uint16_t sport, uint32_t daddr, uint16_t dport) {
  constexpr uint16_t maxlen = sizeof(struct iphdr) + sizeof(struct tcphdr) +
                              /* max TCP options length */ 40;
...
int SynAck::send() {
  if (sendto(raw_socket, buf, psize, 0, (struct sockaddr *)&sin_,
             sizeof(sin_)) < 0) {
...
SynAck::~SynAck() { std::free(buf); }

unsigned char *SynAck::opts() {
  return buf + sizeof(struct iphdr) + sizeof(struct tcphdr);
}

uint16_t mkOptions(unsigned char *buf, uint16_t mss, uint32_t ts_value,
                   uint32_t ts_echo, uint8_t shift_count, bool sack_permitted) {
  uint16_t s = 0;
  if (mss != 0) s += 4;
...
int sendIPv4(iphdr *iph, size_t psize) {
  if (sendto(raw_socket, reinterpret_cast<unsigned char *>(iph), psize, 0,
             (struct sockaddr *)&sin_, sizeof(sin_)) < 0) {
    perror("sendto failed");
    return -1;
  }
  return 0;
...
void relay_packet(iphdr *iph, size_t len, session_data *session) {
  tcphdr *tcph = reinterpret_cast<tcphdr *>(
      reinterpret_cast<unsigned char *>(iph) + iph->ihl * 4);
