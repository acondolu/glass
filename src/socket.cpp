...
int UnixDomainSocket<T, U>::handle_in() {
  int len = recvfrom(socket_fd, buf, sizeof(T) - nbytes, 0, (struct sockaddr *)&from, &fromlen);
  if (len < 0) {
    perror("Socket::recv()");
    return len;
  }
...
int UnixDomainSocket<T, U>::send(U* u) {
  // todo: pad?
  return sendto(socket_fd, (unsigned char*) u, sizeof(U), 0, (struct sockaddr *)&from, fromlen);
}
