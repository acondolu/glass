...
static int cb(struct nfq_q_handle *qh, struct nfgenmsg *nfmsg,
  ...
  printf("/!\\ IP Packet (payload_len=%d) from %s \n", len,
         inet_ntoa(*(in_addr *)&ip_info->saddr));
  if (ip_info->protocol == IPPROTO_TCP) {
    struct tcphdr *tcp_info =
        (struct tcphdr *)((unsigned char *)ip_info + sizeof(struct iphdr));
  ...
  begin:
    ...
    if (flags == TH_SYN) {
      ...
      //    LISTEN --> advance. check state of peer: if SYN_RCVD, reply
      //    with a SYN-ACK (also peer); else, drop SYN_RCVD --> reply with
      //    another SYN-ACK ESTABLISHED --> drop CLOSED --> drop
      //-------------------------------------------------------
      ...
      switch (session->status) {
        case LISTEN: {
          ...
          if (peer != NULL && peer->status == SYN_RCVD) {
            ...
            s->finalize(mkOptions(s->opts(), peer->remote_mss, peer->ts_value,
                                  session->ts_value, peer->shift_count,
                                  peer->sack_permitted));
            s->send();

            peer->syn_ack->tcph->th_seq = session->seq_remote;
            peer->syn_ack->tcph->th_win = session->remote_win;
            peer->syn_ack->finalize(mkOptions(
                peer->syn_ack->opts(), session->remote_mss, session->ts_value,
                peer->ts_value, session->shift_count, session->sack_permitted));
            peer->syn_ack->send();
          }
          return 0;
        }
    ...
    if (session->status == ESTABLISHED && peer != nullptr &&
...
int cb_command(Translator::cmd *cmd) {
  ...
  switch (cmd->tag) {
    ...
    case Translator::EXPIRE_LISTEN: {
      ...
      if (s0->last_activity + Translator::EXPIRE_LISTEN_SECS <= expire->now ||
          s1->last_activity + Translator::EXPIRE_LISTEN_SECS <= expire->now) {
        // FIXME: check more statuses when they will be implemented
        if (s0->status == SYN_RCVD || s0->status == ESTABLISHED)
          state.erase(s0->addr);
        if (s1->status == SYN_RCVD || s1->status == ESTABLISHED)
          state.erase(s1->addr);
        init_map.erase(a0);
        init_map.erase(a1);
        free(s0);
        ...
        expire->timeout(cmd, std::min(s0->last_activity, s1->last_activity) +
                                 Translator::EXPIRE_LISTEN_SECS);
      }
      return 0;
    }
